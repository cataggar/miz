...
| `unsafe_chroot` | root + `CAP_SYS_CHROOT`/`CAP_SYS_ADMIN`/`CAP_MKNOD` | yes, on the host kernel | host's only | install/remove/update packages, configure kernel modules, regenerate initramfs (explicitly or when the packages imply it) |
| `vm` | none | yes, in an isolated guest | any | install/remove/update packages, configure kernel modules, regenerate initramfs (explicitly or when the packages imply it), cross-architecture |
