...
Because the agent is `rdinit`, no kernel module is ever inserted, and **every driver the guest needs must be built into the image's kernel.** The backend reads `lib/modules/<release>/modules.builtin` out of the staged image and refuses the run rather than guessing: an image whose kernel modularizes `ext4`, or both `virtio_blk` and `virtio_scsi`, is rejected in preflight. Azure Linux builds `ext4`, `virtio_scsi` and `virtio_net` in and ships `virtio_blk` as a module, so its disks are attached over virtio-scsi; a kernel with `virtio_blk` built in gets virtio-blk instead. Ubuntu's stock kernel modularizes `ext4` and cannot be booted this way.
