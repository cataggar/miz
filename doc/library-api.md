...
**Both executing backends target the RPM family, and neither is an adapter.** This is policy rather than an accident of what has been implemented so far, and it is worth stating plainly because everything above names the tools one at a time and never adds them up: package transactions are `tdnf` invocations against a `tdnf.conf` this library writes, the installed set is read back with `rpm -qa` and trust material is imported with `rpm --import`, the initramfs is regenerated by `dracut`, the kernel command line is edited in `/etc/default/grub` and regenerated with `grub2-mkconfig`, and SELinux is read from `/etc/selinux/config` and relabelled with `setfiles` against a policy store at the Red Hat-family paths. A Debian or Ubuntu root passes none of those probes and is refused during preflight rather than half-customized.
