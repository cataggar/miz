...
`/.autorelabel` is deliberately not used. It defers the labelling to the target's first boot, which means publishing an image whose first boot relabels and reboots; zvmi's position is that an image is finished when it is published, which is the same reason the relabel happens in the run.
