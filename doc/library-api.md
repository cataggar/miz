...
A package transaction has to resolve repository names, and the target root has no reason to name a resolver that exists wherever this build runs, so something has to supply one. `.packages.resolver` says what: `.host_resolver`, the default, means the build host's own resolver -- `unsafe_chroot` binds the host's `/etc/resolv.conf` into the target root read-only, and the VM backend reaches the same resolver indirectly through QEMU's user-mode networking, which answers on `10.0.2.3` by forwarding to whatever the host uses. `.resolver = .{ .nameservers = &.{"192.0.2.1"} }` states the servers instead, and both backends then render the same `/etc/resolv.conf` bytes from one function, so the resolver stops being a property of where the build ran. At most `MAXNS` (three) dotted-quad addresses are accepted, because a resolver library reads no more than that and a fourth would be a declaration the run never honours. The resolver is the package transaction's, so a request with no package actions installs none at all, and whichever policy is in force the file is removed again when the transaction ends -- it is build-time configuration, not a property of the published image. The choice is in the plan, under the plan hash, and in provenance either way: `host_resolver` is a *declared* inheritance rather than a silent one, and a `unsafe_chroot` run using it also raises a `read_host_resolver` capability naming `/etc/resolv.conf`, so a consumer that requires every input to come from the request can refuse exactly that one.
