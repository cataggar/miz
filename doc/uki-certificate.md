...
`miz uki certificate` extracts the X.509 certificate identified by the
...
```console
miz uki certificate AzureLinux-4.0-x86_64.qcow2 \
  --output release.pem
```
...
```console
miz uki certificate AzureLinux-4.0-aarch64.qcow2 --output=json
```
...
through `miz.Image`. QCOW2 backing files and external data files are rejected
