...
A minimal (~160 KB), statically-linked PID 1 for bootable images built with
`miz build-image --skip-iso-rootfs`. It supports both bare-metal deployments
and virtual machines, including Azure and local QEMU workflows, and serves as a
small reference for what a from-scratch container-image init needs to do.
...
- Runs a PID-1 supervisor loop that never returns and reaps every child.
  ...
  `mizinit.shell=on` explicitly enables a respawning diagnostic root shell on
...
- Distinguishes synthetic local OVF media only by the explicit
  `miz-local-provisioning` marker file. Marked media runs
...
- Supervises exactly one **access provider** — the program that makes the
  ...
  `/usr/local/sbin/mizinit-access` replaces the default with it: mizinit runs
...
- Emits `[mizinit] MIZINIT_PID1_READY supervisor loop active` once PID 1 has
...
Built as part of the repo-root build graph (there's no separate `mizinit/build.zig`):

```
zig build
zig build test-mizinit
```
...
Add the built `zig-out/bin/mizinit` binary to a container image as
`sbin/mizinit`, with relative `sbin/init`, `sbin/poweroff`, `sbin/reboot`, and
`sbin/shutdown` symlinks pointing to `mizinit`, then build an immutable bootable
disk image with:

```
miz build-image --iso <azurelinux.iso> --container <oci-layout-with-mizinit> \
...
```
miz build-image --iso <azurelinux.iso> --container <oci-layout-with-mizinit-agent-sshd> \
  --generation 2 --size 768M --skip-iso-rootfs \
  --extra-kernel-options "init=/sbin/mizinit mizinit.mode=persistent mizinit.azure=auto console=tty0 console=ttyS0,115200n8" \
  -o out.vhd -O vhd
```

`init=/sbin/mizinit` is required when the packaged OpenSSH dependency set includes systemd; otherwise the systemd-based initramfs selects `/usr/lib/systemd/systemd` directly instead of mizinit. Persistent mode is intentionally incompatible with a read-only dm-verity root. If the root remount fails, mizinit leaves provisioning and SSH disabled and retains serial-console access for diagnosis.

`mizinit.azure=auto` is the default and does not infer non-Azure from temporarily missing evidence. Use `mizinit.azure=on` to force provisioning retries when Azure's early-boot signals are unavailable, or `mizinit.azure=off` to suppress `azagent` explicitly. Overrides apply only to the current boot and do not replace the cached automatic decision. `miz azure deprovision` removes `/var/lib/azagent`, including both the provisioning sentinel and cached environment decision.

`mizinit.shell=off` is also the default. Add `mizinit.shell=on` only to a
...
`mizinit.binder=disabled` is the default, and an invalid value is treated the
same way. Add `mizinit.binder=required` to a Binder-capable image's kernel
...
The released `AzureLinux-4.0-*.core.qcow2` images use this `mizinit` +
