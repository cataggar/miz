...
const fixture_volume = iso9660.WriteOptions{
    .volume_id = "VMIZ_SRC",
    .system_id = "VMIZ-SYSTEM",
    .volume_set_id = "VMIZ-SET",
    .publisher_id = "VMIZ-PUBLISHER",
    .preparer_id = "VMIZ-PREPARER",
    .application_id = "VMIZ-APP",
};
...
test "recustomize-iso preserves a dual BIOS+UEFI ISO: nodes/metadata/timestamps except payload, catalog semantics, and nested customized content" {
    ...
    try testing.expectEqualStrings("VMIZ_SRC", report.source_volume.volume_id);
    try testing.expectEqualStrings("VMIZ-SYSTEM", report.source_volume.system_id);
    try testing.expectEqualStrings("VMIZ-PUBLISHER", report.source_volume.publisher_id);
...
test "recustomize-iso preserves an exact UEFI-validation + BIOS-section catalog with non-empty id strings" {
    ...
    @memcpy(validation_id[0..15], "VMIZ-UEFI-VALID");
    var section_id = [_]u8{0} ** 28;
    @memcpy(section_id[0..17], "VMIZ-BIOS-SECTION");
...
test "recustomize-iso verifies output volume metadata on the scratch ISO before publishing" {
    ...
    try testing.expectEqualStrings("VMIZ_SRC", source_meta.volume_id);
