...
/// under it without the plan changing is not a plan. `vmiz oci pin` turns a tag
...
/// is deliberately no arm holding a private key. `vmiz` never touches key
...
pub const UkiSigningProvider = union(enum) {
    ...
    ///
    /// The protocol is the one `vmiz sign` already implements: the unsigned
    /// image, the destination for the signed image, the certificate, and the
    /// digests of the first and third are passed as `VMIZ_UKI_*` environment
    ...
    external_command: struct {
        ...
        /// which is how `vmiz sign` is selected out of the `vmiz` binary.
        argument: []const u8 = "",
    },
};
...
fn validateRegistryImage(
    ...
    switch (registry_reference.selection orelse oci.reference.Selection{ .tag = "latest" }) {
        ...
        .tag => try diagnostics.append(validationError(
            ...
            "run `vmiz oci pin <reference>` and declare the digest it prints",
        )),
    }
...
pub fn resolve(
    ...
    const transaction_name = try std.fmt.allocPrint(plan_allocator, ".vmiz-{s}", .{transaction_hex});
...
/// vmiz runs dracut `--no-hostonly` on both executors, and every path by
...
fn deriveTransactionId(seed: Seed, output_path: []const u8) Uuid {
    ...
    hash.update("vmiz-transaction-id-v1\x00");
...
fn derive(seed: Seed, label: []const u8, index: u64) [32]u8 {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("vmiz-plan-derive-v1\x00");
...
fn hashPlan(plan: ResolvedPlanData) Digest {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("vmiz-resolved-plan-v5\x00");
...
fn hasValidPlanIntegrity(allocator: Allocator, plan: *const ResolvedPlan) Allocator.Error!bool {
    ...
    const transaction_name = try std.fmt.allocPrint(allocator, ".vmiz-{s}", .{transaction_hex});
...
fn hashDirectory(allocator: Allocator, io: Io, path: []const u8) !Digest {
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("vmiz-directory-hash-v1\x00");
...
fn createCustomizeBootTestDisk(io: Io, path: []const u8, spool_path: []const u8) !void {
    ...
    try fat32.format(&image, io, .{ .partition_offset = esp_offset, .partition_len = esp_length });
    {
        ...
        try esp.writeFile(io, "loader/entries/vmiz-1.conf", customize_boot_bls_entry);
    }
...
test "unsafe chroot preflight accepts only the implemented policy subset" {
    ...
    request = baseline;
    request.packages.cache = .{ .cache_only = "/var/cache/vmiz" };
    try std.testing.expectEqual(
        CapabilityState.available,
        try Check.state(&request, .x86_64),
    );
    ...
    request = baseline;
    request.packages.cache = .{ .online_populating = "/var/cache/vmiz" };
    try std.testing.expectEqual(
        CapabilityState.available,
        try Check.state(&request, .x86_64),
    );
...
test "unimplemented typed policies become semantic preflight requirements" {
    ...
    request.packages = .{
        ...
        .cache = .{ .cache_only = "/var/cache/vmiz" },
...
test "a registry image named by tag is refused where it is written" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, diagnostic.remediation.?, "vmiz oci pin") != null);
}
...
test "resolving a registry input twice produces the same plan" {
    ...
    request.input.iso_oci.container.registry.access = .{
        .credential = .{
            .username = "builder",
            .password = .{ .host_environment = "VMIZ_TEST_REGISTRY_TOKEN" },
        },
    };
...
test "the plan hash covers the image and where its credential comes from" {
    ...
    other_variable.input.iso_oci.container.registry.access.credential = .{
        .username = "builder",
        .password = .{ .host_environment = "VMIZ_TEST_OTHER_TOKEN" },
    };
...
test "the plan hash covers which key an image must be signed by" {
    ...
    by_path.input.iso_oci.container.registry.signature = .{
        .key = .{ .host_path = "/etc/vmiz/cosign.pub" },
    };
    ...
    other_path.input.iso_oci.container.registry.signature = .{
        .key = .{ .host_path = "/etc/vmiz/other.pub" },
    };
...
test "a signature key given by path is a capability the plan states" {
    ...
    for (plan.data.required_capabilities) |capability| {
        if (capability.kind == .read_trust_source and
            std.mem.eql(u8, capability.path, "/etc/vmiz/cosign.pub")) found = true;
...
test "a refused signature is reported against the signature, naming the digest and the key" {
    ...
    const by_path: RegistryImage = .{
        ...
        .signature = .{ .key = .{ .host_path = "/etc/vmiz/cosign.pub" } },
    };
    ...
    try std.testing.expect(std.mem.indexOf(u8, diagnostics.items[0].message, "/etc/vmiz/cosign.pub") != null);
    try std.testing.expectEqualStrings(
...
test "a registry input declares registry access instead of a container read" {
    ...
    for (plan.data.required_capabilities) |capability| switch (capability.kind) {
        ...
        .read_host_credential => {
            try std.testing.expectEqualStrings("env:VMIZ_TEST_REGISTRY_TOKEN", capability.path);
            credential_paths += 1;
        },
...
test "native-edit publishes a COSI bundle from a GPT source" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, bundle_bytes, "ID=vmiz") != null);
}
...
test "native-edit appends kernel options to every boot entry on the ESP" {
    ...
    const bls = try readPublishedEspFile(io, output_path, "loader/entries/vmiz-1.conf");
...
test "the chroot backend accepts a kernel argument change and plans the regeneration" {
    ...
    // generated file, so there is nothing for vmiz to append to by hand.
...
test "rebuild execution creates paths and emits strict tree provenance" {
    ...
    try std.testing.expectEqual(ext4.SourceProfile.vmiz_ext4_v1, rebuild_record.profile);
...
test "when_needed is not triggered by kernel-module configuration alone" {
    ...
    // kernel modules should rebuild the initramfs, and it is wrong here: vmiz
...
test "a declared package cache directory must be an absolute normalized path" {
    ...
    var accepted = cacheRequest(.{ .cache_only = "/var/cache/vmiz" });
    var resolved = try resolve(
        std.testing.allocator,
        &accepted,
        .{ .host_architecture = .x86_64 },
...
test "an offline package transaction refuses a declared resolver and the VM backend" {
    ...
    var declared = cacheRequest(.{ .cache_only = "/var/cache/vmiz" });
    ...
    var inherited = cacheRequest(.{ .cache_only = "/var/cache/vmiz" });
    ...
    var guest = cacheRequest(.{ .online_populating = "/var/cache/vmiz" });
...
test "the plan hash covers which cache the packages came from" {
    ...
    const shapes = [_]PackageCachePolicy{
        .online,
        .{ .online_populating = "/var/cache/vmiz" },
        .{ .cache_only = "/var/cache/vmiz" },
...
test "both executing backends declare inheriting the host resolver" {
    ...
    offline.packages.cache = .{ .cache_only = "/var/cache/vmiz" };
    ...
    populating.packages.cache = .{ .online_populating = "/var/cache/vmiz" };
    var populating_resolved = try resolve(
        std.testing.allocator,
        &populating,
        .{ .host_architecture = .x86_64 },
...
test "a repository credential is declared by reference and never by value" {
    ...
    inline for (.{ "request", "plan" }) |which| {
        ...
        const json = output.written();
        try std.testing.expect(
            std.mem.indexOf(u8, json, "VMIZ_REPOSITORY_TOKEN") != null,
        );
    ...
    for (plan.data.required_capabilities) |capability| {
        ...
        found += 1;
        try std.testing.expectEqualStrings("env:VMIZ_REPOSITORY_TOKEN", capability.path);
    }
    ...
    elsewhere[0].credential = .{ .basic = .{
        .username = "builder",
        .password = .{ .host_environment = "VMIZ_OTHER_TOKEN" },
    } };
    var moved = request;
    moved.packages = .{
        .actions = &.{.{ .install = &.{"dracut"} }},
...
test "no credential material reaches any document a run publishes" {
    ...
    const file_material = "vmiz-sentinel-from-a-file";
    const variable_name = "VMIZ_TEST_CREDENTIAL_SENTINEL";
...
test "UKI signing is planned as its own operation, capability, and hash input" {
    ...
    for (plan.data.required_capabilities) |capability| {
        if (capability.kind == .uki_signing_provider) {
            named_provider = true;
            try std.testing.expectEqualStrings("/usr/local/bin/vmiz", capability.path);
        }
    }
