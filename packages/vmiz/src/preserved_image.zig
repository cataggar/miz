...
/// `vmiz_ext4_v1` source profile. `source_date_epoch` controls pure OS
...
pub const RebuildReport = struct {
    ...
    /// only `vmiz_ext4_v1` makes the output a reproducible function of the
...
test "raw mutation hook receives a closed standalone stage and controls publication" {
    ...
    const SuccessHook = struct {
        fn run(
            ...
            try stage.pwrite(hook_io, "vmiz", target.partition.offset + 4096);
        }
    };
    var called = false;
    ...
    try std.testing.expectEqualStrings("vmiz", &changed);
