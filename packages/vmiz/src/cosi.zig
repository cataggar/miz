...
//!
//! Standalone `vmiz cosi` can derive only a subset of that metadata from a raw
...
test "write builds a COSI tarball with GPT metadata and compressed zstd partitions" {
        ...
        .bootloader = .{
            ...
            .systemdBoot = .{
                .entries = &.{
                    .{
                        .type = "uki-standalone",
                        .path = "/boot/efi/EFI/Linux/vmiz.efi",
    ...
    try std.testing.expectEqualStrings("/boot/efi/EFI/Linux/vmiz.efi", parsed_json.value.bootloader.systemdBoot.?.entries[0].path);
