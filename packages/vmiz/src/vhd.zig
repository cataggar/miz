...
pub const creator_app_vmiz: [4]u8 = "vmiz".*;
...
/// Faithful reimplementation of QEMU's `calculate_geometry` so `vmiz`-created
...
pub const Footer = struct {
    ...
    timestamp: u32,
    creator_application: [4]u8 = creator_app_vmiz,
    ...
    /// (including `vmiz` and QEMU's `qem2`) rely on `current_size`. Preserve
