...
const guest_cache_directory = "/run/vmiz-cache";
...
const Session = struct {
    ...
    /// and reboots. vmiz's position is that an image is finished when it is
    ...
    /// is right for an image whose command line only vmiz ever writes. A
...
test "hooks run where the plan says they run, and stop existing when they are done" {
    ...
    try std.testing.expectEqualStrings("/run/vmiz-hook-0", argv[2]);
    ...
    const leftover = try joinGuest(allocator, root_path, "/run/vmiz-hook-0");
    try std.testing.expectError(
        error.FileNotFound,
...
test "a hook that fails fails the run" {
    ...
    try std.testing.expectEqualStrings("/run/vmiz-hook-0", context.timeline.items[0]);
...
test "no credential material reaches anything the run publishes" {
    ...
    const variable_material = "vmiz-sentinel-from-a-variable";
    try Io.Dir.cwd().writeFile(io, .{
        .sub_path = secret_path,
        .data = file_material ++ "\n",
    });
    ...
    const repositories = [_]customize.PackageRepository{
        ...
        .{
            ...
            .credential = .{ .basic = .{
                .username = "builder",
                .password = .{ .host_environment = "VMIZ_TEST_SENTINEL" },
            } },
        },
    };
    const manifest = Manifest{
...
const FakeExecutorContext = struct {
    ...
    fn run(
        ...
        if (std.mem.eql(u8, std.fs.path.basename(argv[0]), "chroot") and
            argv.len >= 3 and
            std.mem.startsWith(u8, argv[2], "/run/vmiz-hook-"))
        {
        ...
        if (containsArg(argv, "/usr/bin/tdnf") and containsArg(argv, "install")) {
            ...
            self.resolver_at_tdnf = self.readTargetFile("run/vmiz-resolv.conf");
            self.repository_at_tdnf = self.readTargetFile("run/vmiz-repos/base.repo");
            self.repository_mode_at_tdnf = self.modeOf("run/vmiz-repos/base.repo");
            self.second_repository_at_tdnf =
                self.readTargetFile("run/vmiz-repos/other.repo");
            self.saw_repository_isolation =
                containsArg(argv, "/run/vmiz-tdnf.conf") and
...
test "a relabel runs last, with the policy the target names" {
    ...
    try std.testing.expectEqualStrings("/run/vmiz-hook-0", timeline[timeline.len - 2]);
