...
pub const control_path = "vmiz-control.json";
/// Path the agent is appended at. `rdinit=/vmiz-guest-agent` matches.
pub const agent_path = "vmiz-guest-agent";
...
pub const kernel_module_config_paths = [_][]const u8{
    "etc/modules-load.d/vmiz.conf",
    "etc/modprobe.d/vmiz-blacklist.conf",
    "etc/modprobe.d/vmiz-options.conf",
};
...
pub const frame_magic = "VMIZVMR1";
...
pub const credential_magic = "VMIZVMK1";
...
test "a control document round-trips and is validated on the way back in" {
    ...
    const control = Control{
        ...
        .modules = &.{ "vmiz-module-00-virtio_pci.ko", "vmiz-module-01-ext4.ko" },
    };
    ...
    try std.testing.expectEqualStrings("vmiz-module-00-virtio_pci.ko", parsed.value.modules[0]);
    try std.testing.expectEqualStrings("vmiz-module-01-ext4.ko", parsed.value.modules[1]);
}
...
test "a control document the guest could be driven by is rejected" {
    ...
    const cases = [_]struct {
        ...
        .{ .expected = error.InvalidModuleMember, .control = blk: {
            var control = base;
            control.modules = &.{"vmiz-module-00-ext4.ko.xz"};
            break :blk control;
        } },
        .{ .expected = error.InvalidModuleMember, .control = blk: {
            var control = base;
...
test "the guest places rendered files only where it recognises the destination" {
    ...
    accepted.kernel_module_files = &.{
        .{ .path = "etc/modules-load.d/vmiz.conf", .contents = "overlay\n" },
        .{ .path = "etc/modprobe.d/vmiz-blacklist.conf", .contents = "blacklist floppy\n" },
    };
    ...
    unknown.kernel_module_files = &.{
        .{ .path = "etc/sudoers.d/vmiz.conf", .contents = "ALL ALL=(ALL) NOPASSWD: ALL\n" },
    };
    ...
    duplicated.kernel_module_files = &.{
        .{ .path = "etc/modules-load.d/vmiz.conf", .contents = "overlay\n" },
        .{ .path = "etc/modules-load.d/vmiz.conf", .contents = "loop\n" },
    };
...
test "rendered files survive the control round trip" {
    ...
    control.kernel_module_files = &.{
        .{ .path = "etc/modprobe.d/vmiz-options.conf", .contents = "options i915 enable_guc=2\n" },
    };
