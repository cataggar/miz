...
///
/// Deliberately covers only what vmiz can actually write today. `ext4` and
...
/// format this partition" enum: adding a variant for a filesystem vmiz cannot
...
pub const FilesystemKind = enum {
    ...
    /// `vmizguest/main.zig` -- answer "which mount type does this kind need"
