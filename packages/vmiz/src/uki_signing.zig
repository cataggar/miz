...
//!
//! vmiz never holds a private key and this module does not change that. It
...
//! `vmiz sign` model, in Azure Artifact Signing, reachable only by a
...
///
/// This is the protocol `vmiz sign` already implements and the Azure Linux
/// release builder already uses, rather than a second one: `VMIZ_UKI_UNSIGNED`,
/// `VMIZ_UKI_SIGNED`, `VMIZ_UKI_CERTIFICATE`, `VMIZ_UKI_UNSIGNED_SHA256` and
/// `VMIZ_UKI_CERTIFICATE_SHA256` name the exchange; `VMIZ_UKI_ARCHITECTURE`
/// and `VMIZ_UKI_SIGNING_METADATA` are advisory.
...
/// What a provider said about itself, in the shape the `vmiz sign` protocol
...
pub const ExternalSignerOptions = struct {
    ...
    /// The image architecture, passed through as `VMIZ_UKI_ARCHITECTURE`.
    ...
    /// build. `vmiz sign` needs exactly that -- a GitHub OIDC request URL and
...
pub const ExternalSigner = struct {
    ...
    /// the form `vmiz sign` uses and that name is safe to repeat. A provider's
    ...
    fn runProvider(
        ...
        environment.put("VMIZ_UKI_UNSIGNED", paths.unsigned) catch return error.OutOfMemory;
        environment.put("VMIZ_UKI_SIGNED", paths.signed) catch return error.OutOfMemory;
        environment.put("VMIZ_UKI_CERTIFICATE", paths.certificate) catch return error.OutOfMemory;
        environment.put("VMIZ_UKI_SIGNING_METADATA", paths.metadata) catch return error.OutOfMemory;
        environment.put("VMIZ_UKI_ARCHITECTURE", self.options.architecture) catch return error.OutOfMemory;
        environment.put("VMIZ_UKI_UNSIGNED_SHA256", &unsigned_hex) catch return error.OutOfMemory;
        environment.put("VMIZ_UKI_CERTIFICATE_SHA256", &certificate_hex) catch return error.OutOfMemory;
...
/// single line in the shape `vmiz sign` emits, whose remainder is a bare error
...
test "only a safe provider error name is repeated" {
    try std.testing.expectEqualStrings(
        ...
        providerErrorName("vmiz sign: failed: ArtifactSigningSubmitFailed\n").?,
    );
    try std.testing.expect(providerErrorName("secret output") == null);
    try std.testing.expect(providerErrorName("vmiz sign: failed: Bad\nInjected") == null);
...
test "an external provider is run over the declared protocol and its result recorded" {
    ...
    const body = try std.fmt.allocPrint(allocator,
        \\{{
        \\  echo "unsigned=$VMIZ_UKI_UNSIGNED"
        \\  echo "signed=$VMIZ_UKI_SIGNED"
        \\  echo "unsigned_sha256=$VMIZ_UKI_UNSIGNED_SHA256"
        \\  echo "certificate_sha256=$VMIZ_UKI_CERTIFICATE_SHA256"
        \\  echo "architecture=$VMIZ_UKI_ARCHITECTURE"
        \\  echo "marker=$VMIZ_TEST_MARKER"
        \\  cat "$VMIZ_UKI_CERTIFICATE"
        \\}} > "{s}/observed.txt"
        \\test -s "$VMIZ_UKI_UNSIGNED"
        \\cp "{s}/presigned.efi" "$VMIZ_UKI_SIGNED"
        \\
    , .{ tmp_path, tmp_path });
    ...
    const forwarded = [_:null]?[*:0]const u8{"VMIZ_TEST_MARKER=forwarded"};
    ...
    const paths = [_][]const u8{ "EFI/Linux/vmiz.efi", "EFI/BOOT/BOOTX64.EFI" };
    ...
    try std.testing.expectEqualStrings("EFI/Linux/vmiz.efi", record.esp_paths[0]);
...
test "a provider that fails or returns the wrong bytes is not taken at its word" {
    ...
    const cases = [_]Case{
        ...
        // A provider that named its failure the way `vmiz sign` does gets that
        // name repeated, so an operator is told what the signing service
        // objected to rather than only that signing stopped.
        .{
            .name = "named.sh",
            .body = "echo 'vmiz sign: failed: ArtifactSigningSubmitFailed' >&2\nexit 1\n",
            .expected = error.SigningProviderFailed,
        ...
        .{
            .name = "passthrough.sh",
            .body = "cp \"$VMIZ_UKI_UNSIGNED\" \"$VMIZ_UKI_SIGNED\"\n",
    ...
    for (cases) |case| {
        ...
        const paths = [_][]const u8{"EFI/Linux/vmiz.efi"};
