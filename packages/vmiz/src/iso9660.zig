...
test "iso writer round-trips nested rock ridge tree with metadata and symlinks" {
    ...
    const nodes = [_]TestNode{
        ...
        .{ .path = "etc/os-release", .kind = .file, .mode = 0o644, .uid = 5, .gid = 6, .bytes = "NAME=vmiz\n" },
    ...
    _ = try writeImagePath(allocator, io, path, ts.source(), .{ .volume_id = "VMIZ_TEST" });
    ...
    defer allocator.free(bytes);
    try std.testing.expectEqualStrings("NAME=vmiz\n", bytes);
...
test "iso writer reproduces an exact boot catalog layout: UEFI validation/default + BIOS section with id strings" {
    ...
    @memcpy(validation_id[0..11], "VMIZ-VALID.");
    var section_id = [_]u8{0} ** 28;
    @memcpy(section_id[0..13], "VMIZ-SECTION.");
...
test "iso9660 writer round-trips volume identifier metadata" {
    ...
    _ = try writeImagePath(allocator, io, path, ts.source(), .{
        ...
        .application_id = "VMIZ ISO WRITER",
    });
    ...
    try std.testing.expectEqualStrings("VMIZ ISO WRITER", meta.application_id);
}
