...
const CredentialDevice = struct {
    ...
    fn create(allocator: Allocator, sealed: []const u8) !CredentialDevice {
        ...
        const rc = std.os.linux.memfd_create("vmiz-credential", 0);
...
fn buildArgv(allocator: Allocator, input: ArgvInput) ![]const []const u8 {
    ...
    switch (policy.network) {
        ...
        .declared_repositories => try argv.appendSlice(&.{
            "-netdev", "user,id=vmiz0",
            "-device", "virtio-net-pci,netdev=vmiz0",
        }),
    }
...
fn appendDisks(
    ...
    switch (input.disk) {
        ...
        .virtio_scsi => {
            try argv.appendSlice(&.{ "-device", "virtio-scsi-pci,id=vmizscsi" });
            for (paths, 0..) |path, index| {
                try argv.appendSlice(&.{ "-drive", if (index == credential_disk_index)
                    try std.fmt.allocPrint(
                        allocator,
                        "file={s},format=raw,if=none,id=vmizdisk{d},{s}",
                        .{ path, index, readonly },
                    )
                else
                    try std.fmt.allocPrint(
                        allocator,
                        "file={s},format=raw,if=none,id=vmizdisk{d},cache=writeback",
                        .{ path, index },
                    ) });
                try argv.appendSlice(&.{ "-device", try std.fmt.allocPrint(
                    allocator,
                    "scsi-hd,drive=vmizdisk{d},bus=vmizscsi.0,channel=0,scsi-id={d},lun=0",
                    .{ index, index },
                ) });
            }
        },
...
test "a software-emulated x86_64 guest is offline and boots the extracted kernel" {
    ...
    try std.testing.expectEqualStrings(
        "console=ttyS0,115200n8 rdinit=/vmiz-guest-agent panic=-1 loglevel=4",
...
test "a kernel with no built-in virtio-blk gets its disks over virtio-scsi" {
    ...
    for (argv) |argument| {
        if (std.mem.eql(u8, argument, "virtio-scsi-pci,id=vmizscsi")) saw_controller = true;
        if (std.mem.eql(
            u8,
            argument,
            "scsi-hd,drive=vmizdisk0,bus=vmizscsi.0,channel=0,scsi-id=0,lun=0",
        )) saw_stage = true;
        if (std.mem.eql(
            u8,
            argument,
            "scsi-hd,drive=vmizdisk1,bus=vmizscsi.0,channel=0,scsi-id=1,lun=0",
        )) saw_result = true;
...
test "a hardware-accelerated aarch64 guest passes the host cpu through and gets a network" {
    ...
    try std.testing.expectEqualStrings("user,id=vmiz0", valueOfArgument(argv, "-netdev").?);
    try std.testing.expectEqualStrings(
        "virtio-net-pci,netdev=vmiz0",
        valueOfArgument(argv, "-device").?,
    );
    try std.testing.expect(indexOfArgument(argv, "-nic") == null);
    try std.testing.expectEqualStrings(
        "console=ttyAMA0,115200n8 rdinit=/vmiz-guest-agent panic=-1 loglevel=4",
        valueOfArgument(argv, "-append").?,
    );
}
