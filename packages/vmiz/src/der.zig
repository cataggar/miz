...
//!
//! Every certificate `vmiz` parses during verification arrives inside the
...
/// vmiz's Secure Boot signing algorithm, so this is the branch that matters.
...
test "no unchecked DER parsers remain in the verifier" {
    ...
    // covers `authenticode.zig`, the only vmiz verifier that parses the
