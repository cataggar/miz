...
//! installer media used by `vmiz build-image`, while keeping the
...
fn decompressZstdAlloc(allocator: std.mem.Allocator, bytes: []const u8, max_size: usize) BlockDecompressionError![]u8 {
    ...
    // packages/vmiz/src/verity_tooling.zig's decompressZstd for why the empty-
...
test "squashfs writer round-trips nested directories, metadata, and a symlink" {
    ...
    const nodes = [_]TestNode{
        ...
        .{ .path = "etc/os-release", .kind = .file, .mode = 0o644, .uid = 1000, .gid = 1001, .content = "NAME=vmiz\n" },
