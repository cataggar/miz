...
/// happened to carry.
pub const config_path = "/run/vmiz-tdnf.conf";
...
pub const repository_directory = "/run/vmiz-repos";
...
pub fn trustPath(allocator: std.mem.Allocator, index: usize) ![]u8 {
    return std.fmt.allocPrint(allocator, "/run/vmiz-trust-{d}.asc", .{index});
}
...
test "the config body differs from itself only by a cache the guest cannot have" {
    ...
    try std.testing.expectEqualStrings(
        "[main]\ngpgcheck=1\nreposdir=/run/vmiz-repos\n",
        without,
    );

    const with = try configBody(allocator, "/run/vmiz-cache");
    defer allocator.free(with);
    try std.testing.expectEqualStrings(
        "[main]\ngpgcheck=1\nreposdir=/run/vmiz-repos\ncachedir=/run/vmiz-cache\nkeepcache=1\n",
        with,
    );
...
test "the paths and the query format are the ones the other side reads back" {
    ...
    try std.testing.expectEqualStrings(
        "/run/vmiz-repos/azurelinux-base.repo",
        repository,
    );
    ...
    try std.testing.expectEqualStrings("/run/vmiz-trust-2.asc", trust);
    ...
    const import = importTrustArgv("/run/vmiz-trust-0.asc");
    try std.testing.expectEqualStrings("/usr/bin/rpm", import[0]);
    try std.testing.expectEqualStrings("--import", import[1]);
    try std.testing.expectEqualStrings("/run/vmiz-trust-0.asc", import[2]);
}
