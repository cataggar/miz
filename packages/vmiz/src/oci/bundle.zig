...
pub fn unpackLayout(
    ...
    try staging.createDir(io, ".vmiz", .default_dir);
    ...
    try writeSyncedFile(io, staging, ".vmiz/base.json", base_json);
    ...
    try writeSyncedFile(io, staging, ".vmiz/metadata.json", metadata_json);
...
fn openPublicationLock(
    ...
    const name = try std.fmt.allocPrint(allocator, ".{s}.vmiz-bundle.lock", .{base});
