...
///
/// Deliberately covers only what miz can actually write today. `ext4` and
...
/// format this partition" enum: adding a variant for a filesystem miz cannot
...
pub const FilesystemKind = enum {
    ...
    /// `mizguest/main.zig` -- answer "which mount type does this kind need"
