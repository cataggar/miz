...
pub const control_path = "miz-control.json";
/// Path the agent is appended at. `rdinit=/miz-guest-agent` matches.
pub const agent_path = "miz-guest-agent";
...
pub const kernel_module_config_paths = [_][]const u8{
    "etc/modules-load.d/miz.conf",
    "etc/modprobe.d/miz-blacklist.conf",
    "etc/modprobe.d/miz-options.conf",
};
...
pub const credential_magic = "MIZVMK1";
...
test "a control document round-trips and is validated on the way back in" {
    ...
    const control = Control{
        ...
        .modules = &.{ "miz-module-00-virtio_pci.ko", "miz-module-01-ext4.ko" },
    };
    ...
    try std.testing.expectEqualStrings("miz-module-00-virtio_pci.ko", parsed.value.modules[0]);
    try std.testing.expectEqualStrings("miz-module-01-ext4.ko", parsed.value.modules[1]);
}
...
test "a control document the guest could be driven by is rejected" {
    ...
    const cases = [_]struct {
        ...
        .{ .expected = error.InvalidModuleMember, .control = blk: {
            var control = base;
            control.modules = &.{"miz-module-00-ext4.ko.xz"};
            break :blk control;
        } },
        .{ .expected = error.InvalidModuleMember, .control = blk: {
            var control = base;
        ...
        .{ .expected = error.TooManyModules, .control = blk: {
            var control = base;
            control.modules = &([_][]const u8{"miz-module-00-ext4.ko"} ** (max_modules + 1));
            break :blk control;
        } },
...
test "the guest places rendered files only where it recognises the destination" {
    ...
    accepted.kernel_module_files = &.{
        .{ .path = "etc/modules-load.d/miz.conf", .contents = "overlay\n" },
        .{ .path = "etc/modprobe.d/miz-blacklist.conf", .contents = "blacklist floppy\n" },
    };
    ...
    unknown.kernel_module_files = &.{
        .{ .path = "etc/sudoers.d/miz.conf", .contents = "ALL ALL=(ALL) NOPASSWD: ALL\n" },
    };
    ...
    duplicated.kernel_module_files = &.{
        .{ .path = "etc/modules-load.d/miz.conf", .contents = "overlay\n" },
        .{ .path = "etc/modules-load.d/miz.conf", .contents = "loop\n" },
    };
...
test "rendered files survive the control round trip" {
    ...
    control.kernel_module_files = &.{
        .{ .path = "etc/modprobe.d/miz-options.conf", .contents = "options i915 enable_guc=2\n" },
    };
