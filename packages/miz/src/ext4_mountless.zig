...
pub const CommitResult = struct {
    ...
    /// miz. Call `deinit()` after recording or using the path.
    recovery_path: []u8,
...
test "atomic commit preserves host image mode timestamps and xattrs" {
    ...
    try setTestHostXattr(io, image_path, "user.miz-host", "preserve-me");
    ...
    const xattr = try readTestHostXattr(allocator, io, image_path, "user.miz-host");
...
test "mountless commit preserves the pinned Ubuntu descriptor-64 profile" {
    ...
    defer source_tree.deinit();
    try source_tree.putDirectory("etc", .{ .mode = 0o755 });
    try source_tree.putFileBytes("etc/os-release", "NAME=miz\n", .{ .mode = 0o644 });
    _ = try ext4.populate(io, image.file, allocator, try source_tree.cursor(), .{
        .length = length,
        .label = "ubuntu-root",
        .uuid = [_]u8{0x71} ** 16,
    ...
    try fs.write("/etc/os-release", "NAME=miz-pinned\n", null);
    var commit_result = try fs.commit();
    defer commit_result.deinit();
    const recovery_path = try allocator.dupe(u8, commit_result.recovery_path);
    defer {
    ...
    try std.testing.expectEqualStrings("NAME=miz-pinned\n", content);
}
