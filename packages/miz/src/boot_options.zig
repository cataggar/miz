...
//! miz-built image live on the ESP -- `EFI/BOOT/grub.cfg`, the vendor
...
/// rather than in anything miz generates, and are recognized because
...
test "an append reaches the default, vendor and BLS copies of the command line" {
    ...
    const image = &target.image;
    {
        ...
        try writeTestFile(&esp, io, "loader/entries/miz-1.conf", test_bls_entry);
    }
    ...
    const bls = try readTestFile(&esp, io, "loader/entries/miz-1.conf");
...
test "an entry that already carries the options is left exactly as it was" {
    ...
    }
    {
        ...
        try before.append(try readTestFile(&esp, io, "loader/entries/miz-1.conf"));
    }
    ...
    var esp = try openTestEsp(&target, io);
    for ([_][]const u8{ "EFI/BOOT/grub.cfg", "loader/entries/miz-1.conf" }, before.items) |file_path, expected| {
...
test "a unified kernel image is refused rather than left carrying the old command line" {
    ...
    const image = &target.image;
    {
        var esp = try openTestEsp(&target, io);
        try writeTestFile(&esp, io, "EFI/BOOT/grub.cfg", test_grub_cfg);
        try writeTestFile(&esp, io, "EFI/Linux/miz-6.1.0.efi", "MZ not-really-a-uki");
    }
