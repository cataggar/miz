...
pub const stdout_scratch_base = "miz-build-image-stdout";
...
pub fn materializeCustomizedRootTree(
    ...
    if (!options.skip_iso_rootfs) {
        ...
        // provides, e.g. mizinit), so wiring in a systemd unit there wouldn't
        // do anything; it's that from-scratch init's own job to invoke
        // azagent if it wants to (see mizinit/README.md).
    ...
    if (options.verity) {
        ...
        switch (try checkInitramfsVerityTooling(allocator, try root_tree.ext4View())) {
            ...
            .inconclusive => emitWarning(
                ...
                "could not conclusively verify that the source initramfs includes dm-verity userspace tooling (systemd-veritysetup-generator/systemd-veritysetup/veritysetup); --verity images built from an initramfs lacking this tooling will hang at boot waiting on /dev/mapper/root (see https://github.com/cataggar/miz/issues/77)",
            ),
        }
    }
...
/// tree rather than being injected by `miz` itself.
...
test "a build reports the kernel command line it composed, including what no plan could state" {
    ...
    var report = try build(allocator, io, .{
        ...
        .size = 256 * mib,
        .verity = true,
        .extra_kernel_options = "console=ttyS0 miz.marker=present",
    });
    defer report.deinit(allocator);
    ...
    try std.testing.expect(std.mem.endsWith(u8, line, "console=ttyS0 miz.marker=present"));
    ...
    try std.testing.expect(std.mem.indexOf(u8, "console=ttyS0 miz.marker=present", root_hash_text) == null);
}
