...
/// `miz_ext4_v1` source profile. `source_date_epoch` controls pure OS
...
pub const RebuildReport = struct {
    ...
    /// only `miz_ext4_v1` makes the output a reproducible function of the
...
test "rebuild inspection validates source without creating artifacts" {
    ...
    try std.testing.expectEqual(ext4.SourceProfile.miz_ext4_v1, inspection.source_profile);
...
test "strict raw rebuild preserves identity tree metadata and outside bytes deterministically" {
    ...
    try std.testing.expectEqual(ext4.SourceProfile.miz_ext4_v1, report.source_profile);
...
test "raw mutation hook receives a closed standalone stage and controls publication" {
    ...
    const SuccessHook = struct {
        fn run(
            ...
            try stage.pwrite(hook_io, "miz", target.partition.offset + 4096);
        }
    };
    var called = false;
    ...
    var output = try Image.openPathReadOnly(io, output_path);
    defer output.close(io);
    var changed: [3]u8 = undefined;
    _ = try output.pread(io, &changed, report.partition_offset + 4096);
    try std.testing.expectEqualStrings("miz", &changed);
