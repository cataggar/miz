...
pub fn writeNoCloudSeed(
    ...
    return writeImage(allocator, io, output, source.treeSource(), .{
        .volume_id = "cidata",
        .application_id = "miz NoCloud seed",
    });
}
...
test "NoCloud seed writer round-trips bounded files through raw and native parsers" {
    ...
    const meta_data = "instance-id: miz-test\nlocal-hostname: miz-test\n";
    ...
    try verifyNoCloudSeedWithEcma119Oracle(bytes, &.{
        ...
        .{ .iso_id = "MIZ_LOCAL_PROVISIONING;1", .contents = "" },
    ...
    for ([_]struct { path: []const u8, contents: []const u8 }{
        ...
        .{ .path = "/miz-local-provisioning", .contents = "" },
    }) |expected| {
...
test "iso writer round-trips nested rock ridge tree with metadata and symlinks" {
    ...
    const nodes = [_]TestNode{
        ...
        .{ .path = "etc/os-release", .kind = .file, .mode = 0o644, .uid = 5, .gid = 6, .bytes = "NAME=miz\n" },
    ...
    _ = try writeImagePath(allocator, io, path, ts.source(), .{ .volume_id = "MIZ_TEST" });
...
test "iso9660 writer round-trips volume identifier metadata" {
    ...
    try std.testing.expectEqualStrings("MIZ ISO WRITER", meta.application_id);
}
