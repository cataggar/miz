...
/// not being true. An image an older miz wrote with 128-byte inodes is
...
/// as a power of two. Images written by an older miz -- and by `mke2fs -I
...
pub const PopulateOptions = struct {
    ...
    /// A source profile to reproduce. The default is miz's compact writer
...
pub const StrictFilesystemIdentity = struct {
    profile: StrictProfile = .miz_ext4_v1,
    uuid: [16]u8,
...
fn validateStrictSuperblock(
    ...
    if (reader.feature_compat != writer_feature_compat or
        ...
        // for byte, and an image an older miz wrote with 128-byte inodes
...
test "populate ext4 and round-trip a small tree with a multi-extent file" {
    ...
    var tree = InMemoryTree.init(&[_]InMemoryEntry{
        ...
        .{ .path = "etc/hostname", .kind = .file, .mode = 0o644, .uid = 1000, .gid = 1000, .size = "miz-test\n".len, .bytes = "miz-test\n" },
    ...
    try std.testing.expectEqualSlices(u8, "miz-test\n", hostname);
...
test "reader opens read-only-safe 64-byte group descriptor ext4 images" {
    ...
    var tree = InMemoryTree.init(&[_]InMemoryEntry{
        .{ .path = "etc", .kind = .directory, .mode = 0o755, .uid = 0, .gid = 0 },
        .{ .path = "etc/os-release", .kind = .file, .mode = 0o644, .uid = 0, .gid = 0, .size = "NAME=miz".len, .bytes = "NAME=miz" },
    });
    tree.bind();
    ...
    try std.testing.expectEqualSlices(u8, "NAME=miz", contents);
}
...
test "the pinned Ubuntu 64-bit profile rebuilds resize and orphan metadata" {
    ...
    try std.testing.expectEqualSlices(u8, "NAME=miz", grown_contents);
}
...
test "populate round-trips xattrs and metadata checksums" {
    ...
    const selinux = [_]Xattr{
        ...
        .{ .name = "user.comment", .value = "hello-from-miz" },
    };
    ...
    var tree = InMemoryTree.init(&[_]InMemoryEntry{
        ...
        .{ .path = "etc/hostname", .kind = .file, .mode = 0o644, .uid = 0, .gid = 0, .size = "miz-test\n".len, .bytes = "miz-test\n", .xattrs = &selinux },
...

    {
        ...
        try expectXattrValue(file_xattrs, "user.comment", "hello-from-miz");
...
test "resize grows ext4 filesystems in place" {
    ...
    try std.testing.expectEqualSlices(u8, "NAME=miz", bytes);
}
...
test "rewriteUuid updates a miz ext4 region and its journal identity" {
    ...
    try std.testing.expectEqual(UuidRewriteProfile.miz_ext4_v1, report.profile);
    try std.testing.expectEqual(SourceProfile.miz_ext4_v1, report.before.profile);
    try std.testing.expectEqual(SourceProfile.miz_ext4_v1, report.after.profile);
...
test "Editor edits (deletes, recursive tree removal, and overwrite) pass a real e2fsck -f check" {
    ...
    // Also confirm the edits themselves are still correct from miz's own
...
fn buildStockFixture(
    ...
    try writeFixtureFile(io, hostname_path, "miz-general\n");
...
test "the general importer reads a stock mke2fs ext4 filesystem in full" {
    ...
    try std.testing.expectEqualStrings("miz-general\n", hostname_bytes);
...
test "the general importer accepts this module's own output and reports its profile" {
    ...
    try std.testing.expect(SourceProfile.miz_ext4_v1.isByteReproducible());
