...
fn decompressZstd(allocator: Allocator, bytes: []const u8, max_size: usize) LoadError![]u8 {
    ...
    // packages/miz/src/verity_tooling.zig's decompressZstd, where the identical
...
test "bundle unpack securely publishes a verified runtime bundle" {
    ...
    defer Io.Dir.cwd().deleteFile(io, ".test-oci-runtime-bundle.miz-bundle.lock") catch {};
    ...
    _ = try published.statFile(io, ".miz/base.json", .{});
    _ = try published.statFile(io, ".miz/metadata.json", .{});
...
test "forced bundle unpack rejects a symlink destination" {
    ...
    defer Io.Dir.cwd().deleteFile(io, ".test-oci-bundle-symlink-destination.miz-bundle.lock") catch {};
...
test "bundle repack publishes deterministic additions changes and whiteouts" {
    ...
    defer Io.Dir.cwd().deleteFile(io, ".test-oci-repack-bundle.miz-bundle.lock") catch {};
    defer Io.Dir.cwd().deleteFile(io, ".test-oci-repack-verify.miz-bundle.lock") catch {};
    defer Io.Dir.cwd().deleteFile(io, ".test-oci-repack-layout.miz-oci-bootstrap.lock") catch {};
