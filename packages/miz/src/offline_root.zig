...
fn namespaceChild(arg: usize) callconv(.c) u8 {
    ...
    if (guest == 0) {
        ...
        childWriteAll(2, "miz-offline-root: exec failed\n");
...
fn childSetupFailure(stage: [*:0]const u8) u8 {
    childWriteAll(2, "miz-offline-root: setup stage '");
...
fn expectNoResidualOfflineMounts(io: Io) !void {
    ...
    while (try iterator.next(io)) |entry| {
        if (std.mem.startsWith(u8, entry.name, "miz-offline-root-"))
...
test "offline root applies structured operations and cleans up" {
    ...
    defer root.deinit();
    try root.apply(&.{
        .{ .create_directory = .{ .path = "/etc/miz", .mode = 0o755 } },
        .{ .write_file = .{ .path = "/etc/miz/config", .source = .{ .inline_bytes = "ok\n" } } },
        .{ .replace_symlink = .{ .path = "/etc/miz/current", .target = "/etc/miz/config" } },
    });
    const config = try root.readFile("/etc/miz/config");
    defer allocator.free(config);
    try std.testing.expectEqualStrings("ok\n", config);
    const link = try root.readLink("/etc/miz/current");
    defer allocator.free(link);
    try std.testing.expectEqualStrings("/etc/miz/config", link);
    const found = try root.discover("/etc/miz", "*");
