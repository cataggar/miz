...
const guest_cache_directory = "/run/miz-cache";
...
const Session = struct {
    ...
    /// and reboots. miz's position is that an image is finished when it is
    ...
    /// is right for an image whose command line only miz ever writes. A
...
const environment_credential_magic = "MIZENV1";
...
test "findRequiredTool returns an error when no candidate exists" {
    try std.testing.expectError(
        ...
        findRequiredTool(std.testing.io, &.{
            "/miz-test/no-such-tool",
            "/miz-test/no-such-tool-either",
        }),
    );
}
...
test "hooks run where the plan says they run, and stop existing when they are done" {
    ...
    try std.testing.expectEqualStrings("/run/miz-hook-0", argv[2]);
    ...
    const leftover = try joinGuest(allocator, root_path, "/run/miz-hook-0");
    try std.testing.expectError(
        error.FileNotFound,
...
test "a hook that fails fails the run" {
    ...
    try std.testing.expectEqualStrings("/run/miz-hook-0", context.timeline.items[0]);
...
test "no credential material reaches anything the run publishes" {
    ...
    const variable_material = "miz-sentinel-from-a-variable";
    try Io.Dir.cwd().writeFile(io, .{
        .sub_path = secret_path,
        .data = file_material ++ "\n",
    });
    ...
    const repositories = [_]customize.PackageRepository{
        ...
        .{
            ...
            .credential = .{ .basic = .{
                .username = "builder",
                .password = .{ .host_environment = "MIZ_TEST_SENTINEL" },
            } },
        },
    };
    const manifest = Manifest{
    ...
    const block = [_:null]?[*:0]const u8{
        "MIZ_TEST_SENTINEL=" ++ variable_material,
    };
...
test "environment credentials use owned pipe storage and are scrubbed when consumed" {
    ...
    const environment_credential = customize.RepositoryCredential{ .basic = .{
        .username = "builder",
        .password = .{ .host_environment = "MIZ_TEST_CREDENTIAL" },
    } };
    ...
    const block = [_:null]?[*:0]const u8{
        "MIZ_TEST_CREDENTIAL=s3cr3t-from-a-variable",
    };
    ...
    try std.testing.expectEqualStrings(
        ...
        std.process.Environ.getPosix(environ, "MIZ_TEST_CREDENTIAL").?,
    );
...
test "worker installs the declared resolver and none without package actions" {
    ...
    const root_path = try temporaryTestPath(allocator, io, &temporary, "root");
    const raw_path = try temporaryTestPath(allocator, io, &temporary, "stage.raw");
    const resolver_path = try joinGuest(allocator, root_path, "/run/miz-resolv.conf");
    const raw_file = try Io.Dir.cwd().createFile(io, raw_path, .{
        .exclusive = true,
        .read = true,
    });
...
const FakeExecutorContext = struct {
    ...
    fn run(
        ...
        if (std.mem.eql(u8, std.fs.path.basename(argv[0]), "chroot") and
            argv.len >= 3 and
            std.mem.startsWith(u8, argv[2], "/run/miz-hook-"))
        {
        ...
        if (containsArg(argv, "/usr/bin/tdnf") and containsArg(argv, "install")) {
            ...
            self.resolver_at_tdnf = self.readTargetFile("run/miz-resolv.conf");
            self.repository_at_tdnf = self.readTargetFile("run/miz-repos/base.repo");
            self.repository_mode_at_tdnf = self.modeOf("run/miz-repos/base.repo");
            self.second_repository_at_tdnf =
                self.readTargetFile("run/miz-repos/other.repo");
            self.saw_repository_isolation =
                containsArg(argv, "/run/miz-tdnf.conf") and
...
test "a relabel runs last, with the policy the target names" {
    ...
    try std.testing.expectEqualStrings("/run/miz-hook-0", timeline[timeline.len - 2]);
