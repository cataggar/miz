...
test "populateEsp copies EFI binaries and generates grub.cfg plus BLS entries" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, grub_cfg, "menuentry 'miz vmlinuz-6.8.12-test' --id 'vmlinuz-6.8.12-test'") != null);
    ...
    try std.testing.expectEqualStrings("miz vmlinuz-6.8.12-test", parsed_bls.title.?);
...
test "populateEsp ignores GRUB's own linux.mod and installer loader binaries as kernel candidates" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, grub_cfg, "menuentry 'miz vmlinuz-6.18.31-1.3.azl4.x86_64'") != null);
...
test "generateBiosGrubCfg renders an MBR grub.cfg for the discovered kernel" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, grub_cfg.bytes, "menuentry 'miz vmlinuz-6.8.12-test' --id 'vmlinuz-6.8.12-test'") != null);
...
test "populateEsp copies MOK assets and emits UKIs when requested" {
    ...
    var tree = InMemoryTree.init(&[_]InMemoryEntry{
        ...
        .{ .path = "EFI/Acme/BOOTX64.CSV", .kind = .file, .bytes = "\"miz\",\"grubx64.efi\",\"shim managed entry\"" },
        .{ .path = "EFI/Acme/MOK.der", .kind = .file, .bytes = "mok-der" },
        .{ .path = "usr/lib/systemd/boot/efi/linuxx64.efi.stub", .kind = .file, .bytes = stub },
        .{ .path = "usr/lib/os-release", .kind = .file, .bytes = "ID=miz\nPRETTY_NAME=\"miz test\"\n" },
    ...
    try std.testing.expectEqualStrings("\"miz\",\"grubx64.efi\",\"shim managed entry\"", boot_csv);
