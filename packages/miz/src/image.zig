...
pub const CreateOptions = struct {
    ...
    /// `miz build-image`/`azure fixup` commands do this automatically).
...
pub const OpenOptions = struct {
    ...
    /// Opt in (`miz --allow-device-write`) to writing through a
...
/// been created with at least `src`'s virtual size). Used by `miz convert`.
