...
pub const VmFirmware = struct {
    ...
    /// the way `miz qemu` does, from the emulator's own `share/` directory
...
test "the cache policy reads both its old spelling and its new one" {
    ...
    const cases = [_]struct {
        ...
        .{ .text = "{\"cache\":{\"online\":{}}}", .expected = .online },
        .{
            .text = "{\"cache\":{\"cache_only\":\"/var/cache/miz\"}}",
            .expected = .cache_only,
            .path = "/var/cache/miz",
        },
        .{
            .text = "{\"cache\":{\"online_populating\":\"/var/cache/miz\"}}",
