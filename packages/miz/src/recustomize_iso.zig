...
const fixture_volume = iso9660.WriteOptions{
    .volume_id = "MIZ_SRC",
    .system_id = "MIZ-SYSTEM",
    .volume_set_id = "MIZ-SET",
    .publisher_id = "MIZ-PUBLISHER",
    .preparer_id = "MIZ-PREPARER",
    .application_id = "MIZ-APP",
};
...
test "recustomize-iso preserves a dual BIOS+UEFI ISO: nodes/metadata/timestamps except payload, catalog semantics, and nested customized content" {
    ...
    try testing.expectEqualStrings("MIZ_SRC", report.source_volume.volume_id);
    try testing.expectEqualStrings("MIZ-SYSTEM", report.source_volume.system_id);
    try testing.expectEqualStrings("MIZ-PUBLISHER", report.source_volume.publisher_id);
    ...
    try testing.expectEqualStrings("miz-live\n", hostname);
...
test "recustomize-iso verifies output volume metadata on the scratch ISO before publishing" {
    ...
    try testing.expectEqualStrings("MIZ_SRC", source_meta.volume_id);
