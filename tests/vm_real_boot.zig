...
//!   MIZ_VM_BOOT_KERNEL=/path/to/vmlinuz-<release>
//!   MIZ_VM_BOOT_MODULES_BUILTIN=/path/to/modules.builtin
//!   MIZ_VM_BOOT_MODULE_TREE=/path/to/lib/modules/<release>   (optional)
//!   MIZ_VM_QEMU=/path/to/qemu-system-<arch>
//!   MIZ_VM_ACCEL=software|hardware        (default: software)
//!   MIZ_VM_BOOT_ARCH=x86_64|aarch64       (default: the host's)
//!   MIZ_VM_BOOT_WORKDIR=/path             (default: /tmp)
//!
...
//!
//! `MIZ_VM_BOOT_MODULE_TREE` points at a real `lib/modules/<release>`, which
...
const hook_phase: miz.customize.HookPhase = .after_packages;
...
const Settings = struct {
    ...
    fn fromEnvironment(environment: *std.process.Environ.Map) ?Settings {
        const requested = environment.get("MIZ_RUN_VM_BOOT_TEST") orelse "";
        if (!std.mem.eql(u8, requested, "1")) {
            std.debug.print(
                "skipping vm real boot: set MIZ_RUN_VM_BOOT_TEST=1 to opt in\n",
                .{},
            );
            return null;
        }
        ...
        const named = environment.get("MIZ_VM_BOOT_ARCH") orelse @tagName(host_architecture);
        const architecture = std.meta.stringToEnum(
            miz.customize.Architecture,
            named,
        ) orelse {
            std.debug.print("skipping vm real boot: unknown MIZ_VM_BOOT_ARCH {s}\n", .{named});
            return null;
        };
        return .{
            .kernel_path = environment.get("MIZ_VM_BOOT_KERNEL") orelse {
                std.debug.print("skipping vm real boot: MIZ_VM_BOOT_KERNEL is unset\n", .{});
                return null;
            },
            .modules_builtin_path = environment.get("MIZ_VM_BOOT_MODULES_BUILTIN") orelse {
                std.debug.print(
                    "skipping vm real boot: MIZ_VM_BOOT_MODULES_BUILTIN is unset\n",
                    .{},
                );
                return null;
            },
            .emulator_path = environment.get("MIZ_VM_QEMU") orelse {
                std.debug.print("skipping vm real boot: MIZ_VM_QEMU is unset\n", .{});
                return null;
            },
            .module_tree_path = environment.get("MIZ_VM_BOOT_MODULE_TREE"),
            ...
            .acceleration = if (std.mem.eql(
                u8,
                environment.get("MIZ_VM_ACCEL") orelse "software",
                "hardware",
            ...
            .work_root = environment.get("MIZ_VM_BOOT_WORKDIR") orelse "/tmp",
        };
    }
};
...
fn runBoot(
    ...
    var outcome = try miz.customize.execute(allocator, io, plan, platform, null);
...
fn createSourceDisk(
    ...
    const kernel = try cwd.readFileAlloc(
        ...
        allocator,
        .limited(miz.vm_payload.max_kernel_bytes),
    );
    ...
    var image = try miz.Image.createExclusive(io, source_path, .raw, disk_size, .{});
