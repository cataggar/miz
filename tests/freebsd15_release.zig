...
test "remote, downloaded, and published releases must match the expectation" {
    ...
    try support.mutateDocument(&tree, release_path, "immutable", "true");
...
test "the publish script requires a reviewed date and exact assets" {
    ...
    try support.expectContains(source, "verify-draft-assets");
    ...
    try support.expectAbsent(source, "--draft >/dev/null 2>&1 || true");
...
test "a published release must report leaving the draft state" {
    ...
    try expectFailure(&context, "published release is not immutable");
