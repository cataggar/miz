...
fn deleteLayout(io: Io, path: []const u8) void {
    ...
    const lock = std.fmt.bufPrint(&lock_buffer, ".{s}.miz-oci-bootstrap.lock", .{base}) catch return;
...
fn deadlineAfterMilliseconds(io: Io, milliseconds: i64) miz.customize.Deadline {
...
test "a stalled registry body cannot outlive the absolute pull deadline" {
    ...
    try std.testing.expectError(
        ...
        miz.customize.pullRegistryImageWithDeadline(
            allocator,
            io,
            std.process.Environ.empty,
...
test "a declared registry image is pulled into the layout the plan named" {
    ...
    var pull = try miz.customize.pullRegistryImage(
        allocator,
        io,
        std.process.Environ.empty,
        .{ .reference = reference, .access = .{ .plain_http = true } },
...
test "a registry needing a credential the request did not declare fails to authenticate" {
    ...
    const reference = try pinnedReferenceFor(allocator, &fixture);
    defer allocator.free(reference);
    try std.testing.expectError(error.AuthenticationFailed, miz.customize.pullRegistryImage(
        allocator,
        io,
        environment,
        .{ .reference = reference, .access = .{ .plain_http = true } },
        ...
        switch (scenario) {
            ...
            .foreign_artifact => try layers.writer.print(
                ...
                .{
                    payload_digest,
                    cosign_payload.len,
                    miz.oci.cosign.signature_annotation,
                    cosign_key_pair_signature,
                },
            ),
            .crowded => for (0..miz.oci.cosign_discovery.max_candidates + 1) |index| {
                if (index != 0) try layers.writer.writeByte(',');
                try layers.writer.print(
                    "{{\"mediaType\":\"{s}\",\"digest\":\"{s}\",\"size\":{d},\"annotations\":{{\"{s}\":\"{s}\"}}}}",
                    .{
                        miz.oci.cosign.payload_media_type,
                        payload_digest,
                        cosign_payload.len,
                        miz.oci.cosign.signature_annotation,
                        cosign_key_pair_signature,
...
fn discoverSignatures(fixture: *SignatureFixture) !miz.oci.cosign_discovery.Outcome {
    var source = try signatureSourceFor(fixture);
    defer source.deinit();
    const image = try oci.content.Digest.parse(cosign_image_digest);
    return miz.oci.cosign_discovery.discover(&source, image);
}
...
test "what discovery returns is what verification accepts" {
    ...
    const key = try miz.oci.cosign.parsePublicKeyPem(cosign_release_public_key);
    const image = try oci.content.Digest.parse(cosign_image_digest);
    ...
    try miz.oci.cosign.verify(allocator, .{
        .payload = candidates[0].payload,
        .signature = candidates[0].signature,
    }, key, image);
    try std.testing.expectError(error.SignatureMismatch, miz.oci.cosign.verify(allocator, .{
...
test "an image with no signature artifact is unsigned, not a failure" {
    ...
    try std.testing.expectEqual(
        miz.oci.cosign_discovery.Absence.no_artifact,
        outcome.absent,
    );
}
...
test "an artifact carrying no simple-signing layer is unsigned, not a parse failure" {
    ...
    try fixture.finish();
    try std.testing.expectEqual(
        miz.oci.cosign_discovery.Absence.no_signature_layer,
        outcome.absent,
    );
}
...
//
// These serve a whole image and a signature over it, so a pull can be run end
// to end against a declared policy. The signature is produced here rather than
// embedded because it has to be over *this* fixture's manifest digest, and a
// digest recomputed by hand in a test file is a constant that silently stops
// matching the moment the fixture's bytes change. The key pair is fixed: the
// PEM below is the real public half of `policy_signing_scalar`, generated by
// OpenSSL, so what the policy parses is a genuine SPKI encoding and not one
// this test invented.
//
// PR #365 already proved the verification itself against cosign's real
// published artifact. What is under test here is the wiring: that a policy is
// consulted, that it is consulted before any layer moves, and that its absence
// changes nothing.

const policy_signing_scalar: [32]u8 = .{
    0xad, 0x60, 0xa1, 0x4f, 0x38, 0xe1, 0x7b, 0x44,
    0xe8, 0x66, 0xeb, 0xa4, 0x3d, 0xa3, 0x1c, 0xd6,
    0x79, 0xf4, 0xa0, 0x1a, 0x6b, 0x82, 0x04, 0x80,
    0xfa, 0x65, 0xef, 0xe2, 0xd7, 0x40, 0x6a, 0x1c,
};

const policy_public_key_pem =
    \\-----BEGIN PUBLIC KEY-----
...
/// A second, unrelated key. Used to state "signed, but not by you", which is a
/// different situation from "not signed" and must not collapse into it.
const policy_other_key_pem =
    \\-----BEGIN PUBLIC KEY-----
    \\MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEeQBpB0M3XN0e5MgPSSGYUxg8wpaZ
    \\PLoIrBqCUcTCbTPEP0S0BW2ZM4CX1zLDBOTPWQ8ULQGMFNTFCwHskYIVKQ==
    \\-----END PUBLIC KEY-----
    \\
;

const PolicySignature = enum { by_declared_key, by_other_key, none };

/// A registry that serves one complete image, and optionally a cosign
/// signature artifact for it.
const PolicyFixture = struct {
    ...
    payload: []u8,
    payload_digest: []u8,
    signature_manifest: []u8,
    signature_manifest_digest: []u8,
    signature_target: []u8,
    /// Set the moment the image's layer leaves for the client. A verification
    /// that happens after this has already spent the transfer it exists to
    /// avoid.
    layer_served: bool = false,
    ...
    fn init(
        ...
        const config = try allocator.dupe(u8, "{\"architecture\":\"amd64\",\"os\":\"linux\"}");
        errdefer allocator.free(config);
        const layer = try allocator.dupe(u8, "a layer nobody should have to download twice");
        errdefer allocator.free(layer);
        const config_digest = try digestText(allocator, config);
        errdefer allocator.free(config_digest);
        const layer_digest = try digestText(allocator, layer);
        ...
        const payload_digest = try digestText(allocator, payload);
        errdefer allocator.free(payload_digest);
        ...
        errdefer allocator.free(signature_manifest);
        ...
        return .{
            ...
            .signature_manifest_digest = signature_manifest_digest,
            .signature_target = signature_target,
        };
    }

    /// Signs `payload` with the fixture's key, or with an unrelated one, or
    ...
    fn signPayload(
        ...
        const produced = try key_pair.sign(payload, null);
        var der_buffer: [Ecdsa.Signature.der_encoded_length_max]u8 = undefined;
        const der = produced.toDer(&der_buffer);
    ...
    fn deinit(self: *PolicyFixture) void {
        ...
        self.allocator.free(self.payload_digest);
        self.allocator.free(self.signature_manifest);
        self.allocator.free(self.signature_manifest_digest);
        self.allocator.free(self.signature_target);
        self.* = undefined;
    }

    fn start(self: *PolicyFixture) !void {
        try self.watchdog.arm(self.io, &.{self.listener.socket});
        errdefer self.watchdog.disarm();
        self.thread = try std.Thread.spawn(.{}, run, .{self});
    }

    fn finish(self: *PolicyFixture) !void {
        if (self.thread) |thread| {
            thread.join();
            self.thread = null;
        }
    ...
    fn run(self: *PolicyFixture) void {
        self.serve() catch |err| {
            self.err = err;
        };
    }

    /// The request count is the measurement, not bookkeeping. A pull refused
    /// for its signature stops before the blobs a successful one fetches, and
    /// the only way to state "it did not download the image" is to say how
    /// many requests it was allowed to make.
    fn serve(self: *PolicyFixture) !void {
        while (self.handled < self.expected_requests) {
            var stream = try self.listener.accept(self.io);
            defer stream.close(self.io);
            var input_buffer: [response_head_buffer_size]u8 = undefined;
    ...
    fn respond(self: *PolicyFixture, request: *std.http.Server.Request) !void {
        const target = request.head.target;
        if (std.mem.eql(u8, target, "/v2/")) return request.respond("", .{});
        ...
        if (std.mem.eql(u8, target, self.signature_target)) {
            ...
            return request.respond(self.signature_manifest, .{ .extra_headers = &.{
                .{ .name = "Content-Type", .value = oci.model.media_type_oci_manifest },
                .{ .name = "Docker-Content-Digest", .value = self.signature_manifest_digest },
            } });
        }

        if (self.matches(target, "manifests", self.manifest_digest)) {
            return request.respond(self.manifest, .{
                .extra_headers = &.{
                    .{ .name = "Content-Type", .value = oci.model.media_type_oci_manifest },
                    .{ .name = "Docker-Content-Digest", .value = self.manifest_digest },
                },
        ...
        if (self.matches(target, "blobs", self.payload_digest)) {
            return request.respond(self.payload, .{ .extra_headers = &.{
                .{ .name = "Content-Type", .value = "application/octet-stream" },
                .{ .name = "Docker-Content-Digest", .value = self.payload_digest },
            } });
        }
        // Not counted as a transfer: `pin` reads the config to learn the
        // image's platform, so it is fetched whether or not a policy exists.
        if (self.matches(target, "blobs", self.config_digest)) {
            return request.respond(self.config, .{ .extra_headers = &.{
                .{ .name = "Content-Type", .value = oci.model.media_type_oci_config },
                .{ .name = "Docker-Content-Digest", .value = self.config_digest },
            } });
        }
        if (self.matches(target, "blobs", self.layer_digest)) {
            self.layer_served = true;
            return request.respond(self.layer, .{ .extra_headers = &.{
                .{ .name = "Content-Type", .value = "application/octet-stream" },
                .{ .name = "Docker-Content-Digest", .value = self.layer_digest },
            } });
        }
        return error.UnexpectedRequestTarget;
    }

    fn matches(self: *PolicyFixture, target: []const u8, kind: []const u8, digest: []const u8) bool {
...
test "a declared signature policy admits an image its key signed" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var fixture = try PolicyFixture.init(allocator, io, .by_declared_key, 10);
    defer fixture.deinit();
    try fixture.start();
    const destination = "test-oci-registry-signature-admitted";
    deleteLayout(io, destination);
    defer deleteLayout(io, destination);
    ...
    // The image landed, and it is the image the reference named.
    try std.testing.expectEqualStrings(fixture.manifest_digest, pull.record.manifest_digest);
    const digest = try oci.content.Digest.parse(fixture.manifest_digest);
    var resolved = try oci.layout.Source.init(io, allocator, destination).resolve(.{
        .path = destination,
...
test "a declared signature policy refuses an unsigned image before it downloads it" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    // Ping, the manifest and config the pin resolves, and the 404 at the
    // signature tag. Nothing after that: the count is how this test states
    // that the refusal came before the transfer rather than after it.
    var fixture = try PolicyFixture.init(allocator, io, .none, 4);
    defer fixture.deinit();
    try fixture.start();
    const destination = "test-oci-registry-signature-unsigned";
    deleteLayout(io, destination);
    defer deleteLayout(io, destination);
    const image_reference = try fixture.reference(allocator);
    defer allocator.free(image_reference);

    try std.testing.expectError(error.RegistryImageUnsigned, miz.customize.pullRegistryImage(
        allocator,
        io,
        std.process.Environ.empty,
        .{
    ...
    try fixture.finish();
    try std.testing.expect(!fixture.layer_served);
}

test "an image signed by somebody else is refused as such, not as unsigned" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var fixture = try PolicyFixture.init(allocator, io, .by_other_key, 5);
    defer fixture.deinit();
    try fixture.start();
    const destination = "test-oci-registry-signature-stranger";
    deleteLayout(io, destination);
    defer deleteLayout(io, destination);
    const image_reference = try fixture.reference(allocator);
    defer allocator.free(image_reference);
    ...
    try std.testing.expectError(error.RegistrySignatureRejected, miz.customize.pullRegistryImage(
        allocator,
        io,
        std.process.Environ.empty,
        .{
...
test "a pull with no signature policy asks the registry nothing about signatures" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    // The regression fence. This feature must not become mandatory by
    // accident: a request that declares no policy makes exactly the requests
    // it made before this existed. A signed artifact is available here and is
    // never asked for -- eight requests against the ten the same fixture
    // serves when a policy is declared, and none of them the signature tag.
    var fixture = try PolicyFixture.init(allocator, io, .by_declared_key, 8);
    defer fixture.deinit();
    try fixture.start();
    const destination = "test-oci-registry-signature-unasked";
    deleteLayout(io, destination);
    defer deleteLayout(io, destination);
    const image_reference = try fixture.reference(allocator);
    defer allocator.free(image_reference);
...
test "provenance for a verified pull is a different value from one that was never checked" {
    ...
    try std.testing.expectEqualStrings(
        &miz.oci.cosign_discovery.signatureTag(image),
...
test "provenance for an unchecked pull does not read as verified" {
    ...
    try std.testing.expectEqual(
        std.meta.Tag(miz.customize.RegistrySignatureRecord).not_requested,
