...
//! helper. Parses a `build-iso` request and drives `zvmi.build_iso.build`,
...
const ParsedArgs = struct {
    ...
    nested_rootfs_path: []const u8 = zvmi.build_iso.default_nested_rootfs_path,
...
pub fn main(init: std.process.Init) !void {
    ...
    const args = parseArgs(arena, argv[1..]) catch |err| {
        std.debug.print("zvmi-iso-builder: {t}\n", .{err});
        std.process.exit(2);
    };

    std.Io.Dir.cwd().createDirPath(io, args.bundle_output_path) catch |err| {
        std.debug.print("zvmi-iso-builder: cannot create result bundle: {t}\n", .{err});
        std.process.exit(1);
    };
    ...
    const customization = customization_loader.load(
        ...
        std.debug.print("zvmi-iso-builder: cannot load customization: {t}\n", .{err});
        std.process.exit(1);
    };
    ...
    const determinism: ?zvmi.build_iso.Determinism = if (args.source_date_epoch) |epoch| .{
    ...
    var report = zvmi.build_iso.build(gpa, io, .{
        ...
        std.debug.print("zvmi-iso-builder: build failed: {t}\n", .{err});
        std.process.exit(1);
    };
    defer report.deinit(gpa);

    writeReport(gpa, io, report_path, report) catch |err| {
        std.debug.print("zvmi-iso-builder: cannot write report: {t}\n", .{err});
        std.process.exit(1);
    };
}
...
fn writeReport(
    ...
    io: std.Io,
    path: []const u8,
    report: zvmi.build_iso.BuildIsoReport,
) !void {
