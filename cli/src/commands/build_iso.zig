//! `miz build-iso --iso <file.iso> --container <oci-layout> --rootfs-size <size> -o <output.iso>`:
...
const help_text =
    \\usage: miz build-iso --iso <file.iso> --container <oci-layout> --rootfs-size <size> -o <output.iso>
...
pub fn run(gpa: std.mem.Allocator, io: std.Io, args: []const []const u8) u8 {
    ...
    var nested_rootfs_path: []const u8 = miz.build_iso.default_nested_rootfs_path;
    ...
    const determinism: ?miz.build_iso.Determinism = if (source_date_epoch) |epoch| .{
...
fn printReport(report: miz.build_iso.BuildIsoReport, dry_run: bool) void {
