//! `miz build-image --iso <file.iso> --container <oci-layout> [--generation 1|2] --size <size> -o <output.{raw|vhd|vhdx|qcow2}|-> [-O raw.gz] [--compress-level <1-9>] [--skip-iso-rootfs] [--verity] [--extra-kernel-options <opts>] [--boot-mode bls|uki|both] [--stub-source-path <path>]`
...
const help_text =
    \\usage: miz build-image --iso <file.iso> --container <oci-layout> [--generation 1|2] --size <size> -o <output.{{raw|vhd|vhdx|qcow2}}|-> [-O raw|raw.gz|raw.zst|vhd|vhdx|qcow2] [--compress-level <1-9>] [--rootfs-path <path>] [--skip-iso-rootfs] [--max-oci-blob-size <size>] [--max-oci-layer-size <size>] [--max-oci-archive-size <size>] [--esp-size <size>] [--ext4-label <label>] [--root-filesystem ext4|xfs] [--journal|--no-journal] [--journal-size <size>] [--root-selinux-label <context>] [--stub-source-path <path>] [--os-release-source-path <path>] [--splash-source-path <path>] [--uki-output-directory <path>] [--verity] [--extra-kernel-options <opts>] [--boot-mode bls|uki|both] [--dry-run] [-v]
    \\
    \\Options:
...
const BuildImageFailureContext = struct {
    ...
    limit_exceeded: ?miz.limits.Exceeded = null,
};
...
pub fn run(gpa: std.mem.Allocator, io: std.Io, args: []const []const u8) u8 {
    ...
    var output_compression: ?miz.output.Compression = null;
    var compression_level: ?miz.output.Level = null;
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, arg, "--iso")) {
            ...
            size = miz.parseSize(args[i]) catch |err|
            ...
            esp_size = miz.parseSize(args[i]) catch |err|
            ...
            compression_level = miz.output.parseLevel(args[i]) catch
    ...
    var report = blk: {
        const built = miz.build_image.build(gpa, io, .{
            ...
            .esp_size = esp_size orelse miz.build_image.default_esp_size,
...
fn printLimitPeaks(peaks: miz.limits.Peaks, configured: miz.limits.ImportLimits) void {
    inline for (comptime std.enums.values(miz.limits.Limit)) |limit| {
...
fn printReport(report: miz.build_image.BuildImageReport, dry_run: bool) void {
    const output_text = (miz.output.Spec{
