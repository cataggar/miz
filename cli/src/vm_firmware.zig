...
//! `miz qemu` resolves its own firmware through, so a firmware boot of a
//! customization run and a hand-driven `miz qemu` run of the same image find
...
fn guestArchitecture(architecture: miz.customize.Architecture) qemu_host.GuestArchitecture {
