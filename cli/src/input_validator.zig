...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len != 2) {
        std.debug.print("usage: miz-input-validator <oci-layout>\n", .{});
        std.process.exit(2);
    }

    var dir = std.Io.Dir.cwd().openDir(init.io, args[1], .{ .iterate = true }) catch |err| {
        std.debug.print("miz-input-validator: cannot open '{s}': {t}\n", .{ args[1], err });
        std.process.exit(1);
    };
    ...
    validateDirectory(init.gpa, init.io, dir) catch |err| {
        if (err == error.UnsupportedEntry) {
            std.debug.print("miz-input-validator: OCI layout '{s}' contains a symlink or special file; only regular files and directories are supported\n", .{args[1]});
        } else {
            std.debug.print("miz-input-validator: cannot validate '{s}': {t}\n", .{ args[1], err });
        }
        std.process.exit(1);
    };
}
