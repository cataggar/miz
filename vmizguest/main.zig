...
//! `rdinit=/vmiz-guest-agent`, so the image's bootloader, firmware, and init
...
//! of vmiz that executes inside a guest whose contents it does not control, and
...
const resolver_backup_path = "/etc/.vmiz-resolv.conf";
...
const Session = struct {
    ...
    /// image vmiz publishes is finished.
    ...
    fn restoreResolver(self: *Session) void {
        ...
        restoreAside(
            ...
            log("[vmiz-guest] could not restore the target resolver\n");
        };
    }
};
...
fn publish(allocator: Allocator, device: []const u8, result: control_mod.Result) void {
    if (device.len == 0) {
        log("[vmiz-guest] no result device; the host will see no answer\n");
        return;
    }
...
fn writeDevice(allocator: Allocator, device: []const u8, bytes: []const u8) void {
    ...
    while (written < bytes.len) {
        ...
        if (linux.errno(rc) != .SUCCESS) {
            log("[vmiz-guest] result device write failed\n");
            return;
        }
        const count: usize = @intCast(rc);
        if (count == 0) break;
...
test "an initramfs is measured by streaming it, not by reading it whole" {
    ...
    const path = "test-vmizguest-initramfs.img";
    ...
    // digest of nothing.
    try std.testing.expectError(error.FileNotFound, measureFile("test-vmizguest-absent.img"));
}
...
test "kernel discovery follows dracut's rule and refuses to find nothing" {
    ...
    const modules_path = "test-vmizguest-modules";
...
test "every directory leading to a rendered destination is created" {
    ...
    // where the file goes.
    const base = "test-vmizguest-parents";
    defer std.Io.Dir.cwd().deleteTree(std.testing.io, base) catch {};
    const path = base ++ "/etc/modprobe.d/vmiz-blacklist.conf";
...
test "the image's own resolver comes back the kind of file it was" {
    ...
    const base = "test-vmizguest-resolver";
    ...
    const backup = base ++ "/etc/.vmiz-resolv.conf";
