...
fn openDebugLog() void {
    const rc = linux.open("/run/vmizinit.log", .{ .ACCMODE = .WRONLY, .CREAT = true, .APPEND = true }, 0o644);
...
fn tryMountEsp() void {
    ...
    while (retry < 5) : (retry += 1) {
        for (candidates) |dev| {
            ...
            if (e == .SUCCESS) {
                ...
                const msg = std.fmt.bufPrint(&buf, "[vmizinit] mounted {s} at /boot/efi\r\n", .{dev}) catch "[vmizinit] mounted ESP\r\n";
                writeStr(msg);
                return;
            }
        }
    ...
    writeStr("[vmizinit] no ESP candidate device mounted (non-fatal)\r\n");
}
...
fn doReboot(cmd: linux.LINUX_REBOOT.CMD) noreturn {
    while (true) {
        ...
        writeErrno("[vmizinit] reboot syscall failed; retrying", linux.errno(rc));
...
fn requestPid1Shutdown(signal: linux.SIG, command: linux.LINUX_REBOOT.CMD) noreturn {
    ...
    if (linux.errno(rc) != .SUCCESS) {
        writeErrno("[vmizinit] failed to signal PID 1", linux.errno(rc));
        linux.exit(1);
    }
    linux.exit(0);
}
...
fn readPersistedHostname(buf: []u8) ?[]const u8 {
    ...
    while (total < buf.len) {
        ...
        if (read_error != .SUCCESS) {
            writeErrno("[vmizinit] reading /etc/hostname failed", read_error);
            return null;
        }
        if (read_rc == 0) break;
        total += read_rc;
    ...
    if (total == buf.len) {
        writeStr("[vmizinit] /etc/hostname is too long; using default hostname\r\n");
        return null;
    }
...
fn ensureMachineId() void {
    ...
    while (random_len < random.len) {
        ...
        if (random_error != .SUCCESS or random_rc == 0) {
            writeErrno("[vmizinit] generating machine-id failed", random_error);
            return;
        }
    ...
    if (linux.errno(fd_rc) != .SUCCESS) {
        writeErrno("[vmizinit] opening /etc/machine-id for writing failed", linux.errno(fd_rc));
        return;
    }
    const fd: i32 = @intCast(fd_rc);
    defer _ = linux.close(fd);
    ...
    while (written < content.len) {
        ...
        if (write_error != .SUCCESS or write_rc == 0) {
            writeErrno("[vmizinit] writing /etc/machine-id failed", write_error);
            return;
        }
        written += write_rc;
    }
    _ = linux.fsync(fd);
    writeStr("[vmizinit] generated /etc/machine-id\r\n");
}
...
fn readBootConfig() BootConfig {
    ...
    if (linux.errno(fd_rc) != .SUCCESS) {
        writeErrno("[vmizinit] opening /proc/cmdline failed; using boot defaults", linux.errno(fd_rc));
        return .{};
    }
    const fd: i32 = @intCast(fd_rc);
    defer _ = linux.close(fd);
    ...
    if (linux.errno(read_rc) != .SUCCESS) {
        writeErrno("[vmizinit] reading /proc/cmdline failed; using boot defaults", linux.errno(read_rc));
        return .{};
    }
    if (read_rc == buf.len) {
        writeStr("[vmizinit] /proc/cmdline is too long; using boot defaults\r\n");
        return .{};
    }
    ...
    if (config.invalid_mode) |value| {
        ...
        const msg = std.fmt.bufPrint(&msg_buf, "[vmizinit] invalid vmizinit.mode={s}; using immutable mode\r\n", .{value}) catch "[vmizinit] invalid vmizinit.mode; using immutable mode\r\n";
        writeStr(msg);
        config.invalid_mode = null;
    }
    if (config.invalid_azure_policy) |value| {
        var msg_buf: [128]u8 = undefined;
        const msg = std.fmt.bufPrint(&msg_buf, "[vmizinit] invalid vmizinit.azure={s}; using auto\r\n", .{value}) catch "[vmizinit] invalid vmizinit.azure; using auto\r\n";
        writeStr(msg);
        config.invalid_azure_policy = null;
    }
...
fn persistAzureDecision(identity: ?[36]u8, decision: AzureDecision) void {
    ...
    if (linux.errno(fd_rc) != .SUCCESS) {
        writeErrno("[vmizinit] opening Azure environment state failed", linux.errno(fd_rc));
        return;
    }
    const fd: i32 = @intCast(fd_rc);
    if (!writeAll(fd, content)) {
        writeStr("[vmizinit] writing Azure environment state failed\r\n");
        _ = linux.close(fd);
    ...
    if (sync_error != .SUCCESS) {
        writeErrno("[vmizinit] syncing Azure environment state failed", sync_error);
        _ = linux.unlink(environment_state_tmp_path);
        return;
    }
    ...
    if (linux.errno(rename_rc) != .SUCCESS) {
        writeErrno("[vmizinit] replacing Azure environment state failed", linux.errno(rename_rc));
        _ = linux.unlink(environment_state_tmp_path);
        return;
    }
...
fn probeProvisioningMedia() provisioning_media.ProbeResult {
    mkdirIgnoreExists("/run/vmizinit");
    mkdirIgnoreExists("/run/vmizinit/provision-media");
    return provisioning_media.probe(
        "/run/vmizinit/provision-media",
        "/run/vmizinit/provision-media/ovf-env.xml",
        "/run/vmizinit/provision-media/" ++ provisioning_media.local_provisioning_marker,
    );
}
...
fn remountRootWritable() bool {
    ...
    const e = linux.errno(rc);
    if (e != .SUCCESS) {
        writeErrno("[vmizinit] remounting / read-write failed", e);
        return false;
    }
    writeStr("[vmizinit] persistent mode: remounted / read-write\r\n");
    return true;
}
...
fn mountEtcOverlay() void {
    ...
    const e = linux.errno(rc);
    if (e != .SUCCESS) {
        writeErrno("[vmizinit] /etc overlay mount failed", e);
        etc_writable = false;
    } else {
        etc_writable = true;
    }
...
// modprobe/kmod binary is shipped either) -- see vmiz issue #88: build-image
...
fn loadModuleAt(gpa: std.mem.Allocator, release: []const u8, rel_path: []const u8) void {
    ...
    const compressed = readWholeFileAlloc(gpa, path) orelse {
        var buf: [128]u8 = undefined;
        const msg = std.fmt.bufPrint(&buf, "[vmizinit] module {s} not found (non-fatal)\r\n", .{rel_path}) catch "[vmizinit] module not found\r\n";
        writeStr(msg);
        return;
    };
    ...
    const image = decompressXzAlloc(gpa, compressed) orelse {
        var buf: [128]u8 = undefined;
        const msg = std.fmt.bufPrint(&buf, "[vmizinit] module {s} decompress failed (non-fatal)\r\n", .{rel_path}) catch "[vmizinit] module decompress failed\r\n";
        writeStr(msg);
        return;
    };
...
fn loadBootModules(mode: BootMode) void {
    ...
    if (linux.errno(linux.uname(&uts)) != .SUCCESS) {
        writeStr("[vmizinit] uname failed; skipping module autoload\r\n");
        return;
    }
...
fn openRawIpRecvSocket(ctl_fd: i32, iface: []const u8) ?i32 {
    const index = ifIndex(ctl_fd, iface) orelse {
        writeStr("[vmizinit] dhcp: SIOCGIFINDEX failed\r\n");
        return null;
    };
    ...
    if (linux.errno(sock_rc) != .SUCCESS) {
        writeErrno("[vmizinit] dhcp: AF_PACKET socket() failed", linux.errno(sock_rc));
        return null;
    }
    ...
    if (linux.errno(bind_rc) != .SUCCESS) {
        writeErrno("[vmizinit] dhcp: AF_PACKET bind failed", linux.errno(bind_rc));
        _ = linux.close(sock);
        return null;
    }
    return sock;
...
fn runDhcp(iface: []const u8) DhcpAttempt {
    ...
    if (linux.errno(hw_rc) != .SUCCESS) writeErrno("[vmizinit] dhcp: SIOCGIFHWADDR failed", linux.errno(hw_rc));
    var mac: [6]u8 = undefined;
    @memcpy(&mac, hw_req.ifru.hwaddr.data[0..6]);
    {
        var mbuf: [64]u8 = undefined;
        const m = std.fmt.bufPrint(&mbuf, "[vmizinit] dhcp: mac={x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}\r\n", .{ mac[0], mac[1], mac[2], mac[3], mac[4], mac[5] }) catch "[vmizinit] dhcp: mac read\r\n";
        writeStr(m);
    }
    ...
    const sock: i32 = @intCast(sock_rc);
    if (linux.errno(sock_rc) != .SUCCESS) {
        writeErrno("[vmizinit] dhcp: socket() failed", linux.errno(sock_rc));
        return result;
    }
    ...
    if (linux.errno(bindtodev_rc) != .SUCCESS) writeErrno("[vmizinit] dhcp: SO_BINDTODEVICE failed", linux.errno(bindtodev_rc));

    const bind_addr = sockaddrInPort(0, std.mem.nativeToBig(u16, 68));
    const bind_rc = linux.bind(sock, &bind_addr, @sizeOf(linux.sockaddr));
    if (linux.errno(bind_rc) != .SUCCESS) {
        writeErrno("[vmizinit] dhcp bind failed", linux.errno(bind_rc));
        return result;
    }
    writeStr("[vmizinit] dhcp: bound to udp/68\r\n");
    ...
    if (openRawIpRecvSocket(ctl_fd, iface)) |raw_sock| {
        ...
        writeStr("[vmizinit] dhcp: falling back to udp recv (may miss replies on some networks)\r\n");
    }
    ...
    const send_rc = linux.sendto(sock, &packet, send_len, 0, &dest_addr, @sizeOf(linux.sockaddr));
    {
        ...
        const smsg = std.fmt.bufPrint(&sbuf, "[vmizinit] dhcp: sent DISCOVER, sendto_rc={d}\r\n", .{send_rc}) catch "[vmizinit] dhcp: sent DISCOVER\r\n";
        writeStr(smsg);
    }
    ...
    while (attempt < 3 and !got_offer) : (attempt += 1) {
        ...
        const n_signed: isize = @bitCast(n_rc);
        {
            ...
            const rmsg = std.fmt.bufPrint(&rbuf, "[vmizinit] dhcp: recvfrom attempt {d} -> {d}\r\n", .{ attempt, n_signed }) catch "[vmizinit] dhcp: recv attempt\r\n";
            writeStr(rmsg);
        }
        ...
        if (payload) |data| {
            if (parseDhcpReply(data, data.len, xid)) |reply| {
                ...
                const pmsg = std.fmt.bufPrint(&pbuf, "[vmizinit] dhcp: parsed reply msg_type={d}\r\n", .{reply.msg_type}) catch "[vmizinit] dhcp: parsed reply\r\n";
                writeStr(pmsg);
                ...
                writeStr("[vmizinit] dhcp: recv'd packet failed to parse (xid/magic mismatch?)\r\n");
            }
        } else {
    ...
    if (!got_offer) {
        writeStr("[vmizinit] dhcp: no offer received\r\n");
        return result;
    }
    ...
    if (got_ack == null) writeStr("[vmizinit] dhcp: no ack received\r\n");
...
fn addDefaultRoute(iface: []const u8, gateway_be: u32) void {
    ...
    const e = linux.errno(rc);
    if (e != .SUCCESS) writeErrno("[vmizinit] add default route failed", e);
}
...
fn writeResolvConf(dns: [2]u32) void {
    ...
    const fd: i32 = @intCast(fd_rc);
    if (linux.errno(fd_rc) != .SUCCESS) {
        writeErrno("[vmizinit] writing resolv.conf failed", linux.errno(fd_rc));
        return;
    }
    defer _ = linux.close(fd);
    if (!etc_writable) writeStr("[vmizinit] /etc not writable (no overlayfs); wrote /run/resolv.conf instead\r\n");
...
///
/// vmizinit supervises exactly one program that makes the machine reachable.
...
fn spawnAzagent(mode: AzagentLaunchMode) ?linux.pid_t {
    ...
    writeStr("[vmizinit] running azagent...\r\n");
    const pid = forkProcess("[vmizinit] fork() for azagent failed") orelse return null;
    if (pid == 0) {
        attachChildLog();
        ...
        writeStr("[vmizinit] execve(azagent) failed\r\n");
        linux.exit(127);
    }
    return pid;
}
...
fn spawnDiagnosticShell(console_path: [*:0]const u8) ?linux.pid_t {
    ...
    if (linux.errno(tty_fd_raw) != .SUCCESS) {
        writeErrno("[vmizinit] opening diagnostic serial console failed", linux.errno(tty_fd_raw));
        return null;
    }
    const tty_fd: i32 = @intCast(tty_fd_raw);
    const pid = forkProcess("[vmizinit] fork() for diagnostic shell failed") orelse {
    ...
    if (pid == 0) {
        ...
        writeStr("[vmizinit] execve(/usr/bin/bash) failed\r\n");
        linux.exit(127);
    }
    _ = linux.close(tty_fd);
    return pid;
...
fn logAzureDecision(decision: AzureDecision, policy: AzurePolicy, evidence: AzureEvidence, cached: ?AzureDecision) void {
    switch (policy) {
        .on => writeStr("[vmizinit] Azure environment forced on by vmizinit.azure=on\r\n"),
        .off => writeStr("[vmizinit] Azure environment forced off by vmizinit.azure=off\r\n"),
        .auto => switch (decision) {
            .azure => if (evidence.saw_option_245)
                writeStr("[vmizinit] Azure environment detected from DHCP option 245\r\n")
            else if (evidence.media == .azure)
                writeStr("[vmizinit] Azure environment detected from provisioning media\r\n")
            else if (cached == .azure)
                writeStr("[vmizinit] Azure environment restored from persistent state\r\n")
            else
                writeStr("[vmizinit] Azure environment detected\r\n"),
            .local => writeStr("[vmizinit] explicit local provisioning media detected; WireServer Ready will be skipped\r\n"),
            .non_azure => if (cached == .non_azure)
                writeStr("[vmizinit] non-Azure environment restored from persistent state; skipping azagent\r\n")
            else
                writeStr("[vmizinit] non-Azure environment detected; skipping azagent\r\n"),
            .unknown => writeStr("[vmizinit] Azure environment is unknown; retrying detection\r\n"),
        },
    }
}
...
fn noteChildExit(supervisor: *Supervisor, pid: linux.pid_t, status: u32) void {
    if (pid == supervisor.azagent_pid) {
        ...
        if (childSucceeded(status)) {
            ...
            writeStr("[vmizinit] azagent completed successfully\r\n");
        } else {
            supervisor.azagent_retry = provisioning_retry_seconds;
            writeStr("[vmizinit] azagent failed; retrying in 5 seconds\r\n");
        }
        ...
        writeStr("[vmizinit] access provider exited unexpectedly; scheduling restart\r\n");
    } else if (pid == supervisor.shell_pid) {
        supervisor.shell_pid = 0;
        supervisor.shell_retry = 1;
        writeStr("[vmizinit] diagnostic shell exited; scheduling restart\r\n");
    } else {
        writeStr("[vmizinit] reaped adopted child\r\n");
    }
}
...
fn reapChildren(supervisor: *Supervisor) void {
    while (true) {
        ...
        if (wait_error != .SUCCESS) {
            writeErrno("[vmizinit] waitpid() failed", wait_error);
            return;
        }
...
fn drainExitedChildren(supervisor: *Supervisor) ChildDrainState {
    while (true) {
        ...
        if (wait_error != .SUCCESS) {
            writeErrno("[vmizinit] shutdown waitpid() failed", wait_error);
            return .remaining;
        }
        noteChildExit(supervisor, @intCast(wait_rc), status);
    }
...
fn reapUntilNoChildren(supervisor: *Supervisor) void {
    while (true) {
        ...
        writeErrno("[vmizinit] final shutdown waitpid() failed; retrying", wait_error);
        const req: linux.timespec = .{ .sec = 1, .nsec = 0 };
        _ = linux.nanosleep(&req, null);
    }
}
...
fn broadcastGuestSignal(signal: linux.SIG) void {
    if (!isPid1(linux.getpid())) {
        writeStr("[vmizinit] refusing shutdown broadcast outside PID 1\r\n");
        linux.exit(1);
    }
    ...
    if (signal_error != .SUCCESS and signal_error != .SRCH) {
        writeErrno("[vmizinit] broadcast shutdown signal failed", signal_error);
    }
}

fn shutdownSupervisor(supervisor: *Supervisor) noreturn {
    writeStr("[vmizinit] terminating all guest processes\r\n");
    var phase: ShutdownPhase = .term;
    broadcastGuestSignal(.TERM);
    phase = phaseAfterDrain(phase, boundedChildDrain(supervisor, 50));
    if (phase == .kill) {
        writeStr("[vmizinit] forcing remaining guest processes to exit\r\n");
        broadcastGuestSignal(.KILL);
        phase = phaseAfterDrain(phase, boundedChildDrain(supervisor, 50));
    }
    if (phase == .final_reap) {
        writeStr("[vmizinit] waiting for all killed children to be reaped\r\n");
...
fn supervisorLoop(supervisor: *Supervisor) noreturn {
    if (!isPid1(linux.getpid())) {
        writeStr("[vmizinit] fatal: supervisor must run as PID 1\r\n");
        linux.exit(1);
    }
    if (supervisor.shell_enabled) {
        var buf: [128]u8 = undefined;
        const path = std.mem.span(supervisor.console_path);
        const msg = std.fmt.bufPrint(&buf, "[vmizinit] diagnostic root shell enabled on {s}\r\n", .{path}) catch "[vmizinit] diagnostic root shell enabled\r\n";
        writeStr(msg);
    } else {
        ...
        if (shouldRunAzagent(supervisor)) {
            if (spawnAzagent(azagentLaunchMode(supervisor.decision).?)) |pid| {
                ...
                if (!supervisor.azagent_missing_logged) {
                    writeStr("[vmizinit] provisioning requires /usr/sbin/azagent; SSH remains gated\r\n");
...
pub fn main(init: std.process.Init.Minimal) noreturn {
    ...
    writeStr("\r\n[vmizinit] base mounts ready; configuring network...\r\n");
    const network = setupNetworking();

    if (boot_mode == .persistent and !persistent_root_ready) {
        writeStr("[vmizinit] persistent storage is unavailable; azagent and SSH will not start\r\n");
    }
...
test "parseBootConfig accepts boot modes Azure policies and explicit shell setting independently" {
    const persistent = parseBootConfig("root=/dev/sda2 vmizinit.mode=persistent vmizinit.azure=on vmizinit.shell=on console=ttyS0");
    ...
    const immutable = parseBootConfig("vmizinit.mode=immutable vmizinit.azure=off vmizinit.shell=off");
...
test "parseBootConfig allows later valid values to replace invalid ones" {
    const config = parseBootConfig("vmizinit.mode=invalid vmizinit.azure=maybe vmizinit.shell=yes vmizinit.mode=persistent vmizinit.azure=on vmizinit.shell=on");
    try std.testing.expectEqual(BootMode.persistent, config.mode);
    try std.testing.expectEqual(AzurePolicy.on, config.azure_policy);
    try std.testing.expect(config.shell_enabled);
    try std.testing.expectEqual(@as(?[]const u8, null), config.invalid_mode);
...
test "parseBootConfig rejects invalid values with safe defaults" {
    const config = parseBootConfig("vmizinit.mode=writable vmizinit.azure=maybe vmizinit.shell=yes");
    try std.testing.expectEqual(BootMode.immutable, config.mode);
    try std.testing.expectEqual(AzurePolicy.auto, config.azure_policy);
    try std.testing.expect(!config.shell_enabled);
