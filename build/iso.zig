...
//! consumer's build. Runs the host `miz-iso-builder` over a source ISO and an
...
pub fn add(
    ...
    const run = b.addRunArtifact(dependency.artifact("miz-iso-builder"));
...
pub fn addRecustomize(
    ...
    const run = b.addRunArtifact(dependency.artifact("miz-recustomize-iso-builder"));
